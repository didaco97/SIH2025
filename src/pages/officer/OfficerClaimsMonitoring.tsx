...
import { getAllClaims, updateClaimStatus, ClaimData, ClaimStatus } from "@/lib/firestore";
      ...
      <main className="ml-64 p-6 overflow-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="stat-card">
            ...
            <p className="text-2xl font-bold text-foreground">{stats.total > 0 ? Math.round((stats.approved / stats.total) * 100) : 0}%</p>
          </div>
          <div className="stat-card">
            <div className="flex items-center justify-between mb-2">
          ...
          <div className="stat-card">
            ...
            <p className="text-sm text-muted-foreground">Fraud Risk Factor</p>
            <p className="text-2xl font-bold text-foreground">High (12%)</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-card rounded-2xl shadow-card p-5">
            <h3 className="text-lg font-semibold text-foreground font-display mb-4">Claim Trends vs Approvals</h3>
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={claimTrendsData}>
                ...
                <Area type="monotone" dataKey="claims" stroke="hsl(200, 70%, 50%)" fill="url(#claimsGradient)" strokeWidth={2} name="Total Claims" />
          ...
          {/* Claims by Loss Type */}
          <div className="bg-card rounded-2xl shadow-card p-5">
            <h3 className="text-lg font-semibold text-foreground font-display mb-4">Claims by Loss Type</h3>
        ...
        {/* Second Charts Row */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          {/* Risk Analysis by District */}
          <div className="bg-card rounded-2xl shadow-card p-5">
            <h3 className="text-lg font-semibold text-foreground font-display mb-4">High Risk Districts</h3>
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={riskAnalysisData} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} stroke="hsl(120, 15%, 88%)" />
                <XAxis type="number" domain={[0, 100]} hide />
                <YAxis dataKey="district" type="category" width={80} tick={{ fontSize: 10 }} stroke="hsl(150, 10%, 45%)" />
                <Tooltip cursor={{ fill: 'transparent' }} contentStyle={{ backgroundColor: 'hsl(0, 0%, 100%)', border: '1px solid hsl(120, 15%, 88%)', borderRadius: '8px' }} />
                <Bar dataKey="risk" radius={[0, 4, 4, 0]} barSize={16} name="Risk Score">
                  {riskAnalysisData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={entry.risk > 80 ? "hsl(10, 80%, 55%)" : entry.risk > 60 ? "hsl(45, 90%, 55%)" : "hsl(142, 60%, 45%)"} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          {/* Flagged Claims */}
          <div className="bg-card rounded-2xl shadow-card p-5 overflow-hidden">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-lg font-semibold text-foreground font-display">Flagged Claims</h3>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left py-1.5 font-medium text-muted-foreground">ID</th>
                    <th className="text-left py-1.5 font-medium text-muted-foreground">Reason</th>
                    <th className="text-left py-1.5 font-medium text-muted-foreground">Risk</th>
                    <th className="text-right py-1.5 font-medium text-muted-foreground">Action</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className="border-b border-border/50">
                    <td className="py-2 font-medium">#CLM-9021</td>
                    <td className="py-2 text-muted-foreground">Location Mismatch</td>
                    <td className="py-2 text-danger-zone font-bold">High</td>
                    <td className="py-2 text-right"><Button size="sm" variant="ghost" className="h-6 text-xs text-primary">Review</Button></td>
                  </tr>
                  <tr className="border-b border-border/50">
                    <td className="py-2 font-medium">#CLM-9034</td>
                    <td className="py-2 text-muted-foreground">Duplicate Photo</td>
                    <td className="py-2 text-danger-zone font-bold">High</td>
                    <td className="py-2 text-right"><Button size="sm" variant="ghost" className="h-6 text-xs text-primary">Review</Button></td>
                  </tr>
                  <tr className="border-b border-border/50">
                    <td className="py-2 font-medium">#CLM-8812</td>
                    <td className="py-2 text-muted-foreground">Area Discrepancy</td>
                    <td className="py-2 text-golden font-bold">Med</td>
                    <td className="py-2 text-right"><Button size="sm" variant="ghost" className="h-6 text-xs text-primary">Review</Button></td>
                  </tr>
                  <tr>
                    <td className="py-2 font-medium">#CLM-9100</td>
                    <td className="py-2 text-muted-foreground">Multiple Claims</td>
                    <td className="py-2 text-golden font-bold">Med</td>
                    <td className="py-2 text-right"><Button size="sm" variant="ghost" className="h-6 text-xs text-primary">Review</Button></td>
                  </tr>
                </tbody>
        ...
        <div className="bg-card rounded-2xl shadow-card overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">Claims filed by farmers will appear here</p>
            </div>
          ) : (
            <>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-secondary/50">
                    <tr>
                      ...
                      <th className="text-left py-4 px-4 font-semibold text-foreground text-sm">Farmer ID</th>
                      ...
                      <th className="text-left py-4 px-4 font-semibold text-foreground text-sm">Filed By</th>
                    ...
                    {filteredClaims.map((claim, index) => {
                      ...
                      return (
                        <tr key={claim.id} className={`border-b border-border ${index % 2 === 0 ? 'bg-background' : 'bg-card'}`}>
                          ...
                          <td className="py-4 px-4">{claim.farmerName}</td>
                          <td className="py-4 px-4 text-muted-foreground font-mono text-xs">{claim.farmerId?.slice(0, 12)}...</td>
                          ...
                          <td className="py-4 px-4">
                            <span className={`text-xs px-2 py-1 rounded-full ${claim.filedBy === 'officer'
